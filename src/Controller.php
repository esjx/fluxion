...
class Controller extends Service
{
    ...
    public function setup($arr)
    {
        ...
        foreach (Util::loadAllFiles($this->dir . DIRECTORY_SEPARATOR . 'Models') as $file) {
            ...
            if (is_subclass_of($file, Model::class) && !$test->isAbstract()) {

                /** @var Model $model */
                $model = Model::createFromName($file, $config, $auth);
        ...
        foreach ($list as $class=>$index) {
            ...
            if (is_subclass_of($class, Model::class) && !$test->isAbstract()) {

                $model = Model::createFromName($class, $config, $auth);
    ...
    public function data($arr)
    {
        ...
        $model->changeState(Model::STATE_FILTER);
    ...
    public function fields($arr)
    {
        ...
        $model->changeState(Model::STATE_EDIT);
    ...
    public function excelFast($arr, $total)
    {
        ...
        $model->changeState(Model::STATE_EXCEL);
                                ...
                                default:
                                    ...
                                    if (isset($value['choices'])) {
                                        ...
                                        if (isset($value['choices_colors'])
                                            ...
                                            if (isset(Model::COLOR_MAP[$color])) {

                                                $cor->getFont()->setColor(new Excel\Style\Color('FF' . strtoupper(Model::COLOR_MAP[$color])));
    ...
    public function createModelFromRoute($arr, string $permission = 'view'): Model
    {
    ...
    public function typeahead($arr)
    {
        ...
        $m->changeState(Model::STATE_TYPEAHEAD);
