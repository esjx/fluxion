...
class ModelManipulate
{
     ...
     * @return bool
     */
    public static function sync($model, $drop, Config $config, Auth $auth) {

        try {

            $class_model = Model::createFromName($model, $config, $auth);
            ...
            // Apagar a tabela atual, se for a opção do usuário ($drop = true)
            if ($drop) {
